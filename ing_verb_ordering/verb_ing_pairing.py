...
import pickle
from data_processing import extract_info_1M_np as info
...
def initialize_with_pretrained(pretrained_embeds, word_embedding, word_to_ix, use_cuda=False):
...
def train(emb_dim, hidden_dim):
    # Prepare data
    data_info = info.OneMInfo()
    word_to_ix = {}
    tag_to_ix = {}

    # Prepare Model
    pret_embs = pickle.load(open('data/pretrained-embeds-coref.pkl', 'rb'))
    model = LSTMTagger(emb_dim, hidden_dim, len(word_to_ix), len(tag_to_ix))
    initialize_with_pretrained(pret_embs, model, word_to_ix)
