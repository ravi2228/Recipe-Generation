...
from ing_verb_ordering.constants import TRAIN_FILE, DEV_FILE, TEST_FILE
    ...
    with open(input_file, 'r') as in_file:
        for line in in_file.readlines():
            ...
            if line.strip() != '':
                ...
                if len(pairs) > 0:
                    title = pairs[0]
                    X.append([title.strip().lower()])
                    Y.append([title.strip().lower()])
                    for pair in pairs[1:]:
                        if pair.strip() != '':
                            ing_verb = pair.split('::')
                            if len(ing_verb) == 2:
...
def read_langs_ing_verb(data_file, reverse=False):
    print("Reading lines...")
    title, X, Y = load_data(data_file)
    # Read the file and split into lines
    #lines = open('%s-%s.txt' % ('ing', lang2), encoding='utf-8').read().strip().split('\n')
    lang1 = 'ing'
    lang2 = 'verb'
    # Split every line into pairs and normalize
    # for z in range(len(X)):
    #     strX = normalize_string(' '.join(X[z]))
    #     strY = normalize_string(' '.join(Y[z]))

    pairs = [[normalize_string(' '.join(X[z])), normalize_string(' '.join(Y[z]))] for z in range(len(X))]
...
def prepare_data_ing_verb(data_file, reverse=False):
    input_lang, output_lang, pairs = read_langs_ing_verb(data_file, reverse)
    print("Read %s sentence pairs" % len(pairs))
...
input_lang, output_lang, pairs = prepare_data_ing_verb(TRAIN_FILE, True)
...
class Attn(nn.Module):
    ...
    def score(self, hidden, encoder_output):
        ...
        if self.method == 'dot':
            ...
            energy = torch.mm(hidden, energy.t())
            return energy
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            decoder_output, decoder_context, decoder_hidden, decoder_attention = decoder(decoder_input, decoder_context,
                                                                                         decoder_hidden,
                                                                                         encoder_outputs)
            # print(list(decoder_output[0].size()),list(target_variable[di].size()))
            # d = decoder_output[0].unsqueeze(0)
            # print(resize2d(decoder_output[0], (1,decoder_output[0].size())).size())
            # d = transform.Resize(decoder_output[0], (1, 1733))
            # print(d.size())
        ...
        for di in range(target_length):
            ...
            # print(resize2d(decoder_output[0], (1,decoder_output[0].size())).size())
            # print(transform.Resize(decoder_output[0], (1, 1733)).size())
            # d = decoder_output[0].unsqueeze(0)
            # print(resize2d(decoder_output[0], (1,decoder_output[0].size())).size())
            # d = transform.Resize(decoder_output[0], (1, 1733))
            # print(d.size())
...
def initialize_with_pretrained(pretrained_embeds, word_embedding, lang, use_cuda=USE_CUDA):
...
initialize_with_pretrained(pret_embs, encoder, input_lang)
initialize_with_pretrained(pret_embs, decoder, output_lang)
# Move models to GPU
if USE_CUDA:
    encoder.cuda()
    decoder.cuda()
...
def evaluate(sentence, max_length=MAX_LENGTH):
    ...
    for di in range(max_length):
        decoder_output, decoder_context, decoder_hidden, decoder_attention = decoder(decoder_input, decoder_context,
                                                                                     decoder_hidden, encoder_outputs)
        # print(decoder_attention.squeeze(0).squeeze(0).cpu().data.size())
        # print(decoder_attentions[di,:decoder_attention.size(2)].size())
